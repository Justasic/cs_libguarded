	...
	namespace detail
	{
		...
		class guard_locker_base
		{
		  public:
			...
			[[nodiscard]] bool         operator<(const guard_locker_base &rhs) const noexcept { return m_address < rhs.m_address; }
		...
		class guard_locker: public guard_locker_base
		{
		  public:
			...
			void                    reset() override { m_lock.reset(); }

			[[nodiscard]] lock_type take_lock() && { return std::move(m_lock); }
		...
		class guard_reader
		{
		  public:
			...
			[[nodiscard]] auto try_lock() { return m_guard.try_lock_shared(); }
