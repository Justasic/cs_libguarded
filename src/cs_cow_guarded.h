...
namespace libguarded
{
	...
	*/
	  ...
	  private:
		class deleter
		{
		  public:
			...
			deleter(std::unique_lock<Mutex> &&lock, cow_guarded &guarded): m_lock(std::move(lock)), m_guarded(&guarded), m_cancelled(false) {}
	...
	template<typename... Us>
	cow_guarded<T, M>::cow_guarded(Us &&...data): m_data(std::make_shared<T>(std::forward<Us>(data)...))
	{}
